...
`look_back = TRUE` leads to additional rejections. Two key changes are made:
...
With `look_back = FALSE`, neither $H_4$ nor $H_5$ is rejected:

```{r oncology-look-back-diff-no}
print(result_onc_no_lb)
```

With `look_back = TRUE`, both $H_4$ and $H_5$ **are rejected** and
...
the rejection. Similarly, $H_5$ starts with weight 0.02 and boundary
0.0005 — too small for its p-value of 0.0008. But after $H_4$'s
rejection propagates weight to $H_5$, looking back to analysis 1 with
the increased weight enables $H_5$'s rejection as well.
