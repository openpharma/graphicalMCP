---
title: "Graphical Approaches in Group Sequential Design"
output:
  rmarkdown::html_vignette:
    number_sections: true
    toc: true
vignette: >
  %\VignetteIndexEntry{Graphical Approaches in Group Sequential Design}
...
Note that wrapping only borrows `gsDesign`'s spending formulas; the
boundaries and repeated p-values are still computed by `graphicalMCP`. To
delegate the group sequential calculations themselves to `gsDesign`, use
`graph_test_shortcut_gsDesign()`, which takes `gsDesign`'s spending functions,
parameters, and spending time (`sfu`, `sfupar`, `usTime`) directly and is
described in the
[Graphical Approaches in Group Sequential Design: Combining graphicalMCP and gsDesign](group-sequential-testing-gsDesign.html)
vignette.
